    ...
    const result = secrets
      ...
      .map((_) => {
        ...
        if (
          isNaN(num) ||
          num < 0 ||
          _.length !== this.versionLength + this.secretLength
        )
          return "";
        return _;
      })
  ...
  decrypt = async (
    ...
    try {
      ...
      const prefix = ciphertext.substring(
        0,
        this.versionLength + this.randomLength + this.authTagLength
      );
      ...
      const authtag = prefix.substr(
        this.versionLength + this.randomLength,
        this.authTagLength
      );

      const restSecret = ciphertext.substr(
        this.versionLength + this.randomLength + this.authTagLength
      );
